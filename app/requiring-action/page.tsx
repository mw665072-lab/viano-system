...
function getTagStyle(alertTier: string, percentageUsed: number) {
  ...
  if (isTier2) {
    return { bg: "bg-red-50 dark:bg-red-500/15", text: "text-red-600 dark:text-red-400", border: "border-red-200 dark:border-red-500/30" }
  }
  if (percentageUsed >= 80) {
    return { bg: "bg-orange-50 dark:bg-orange-500/15", text: "text-orange-600 dark:text-orange-400", border: "border-orange-200 dark:border-orange-500/30" }
  }
  if (percentageUsed >= 60) {
    return { bg: "bg-amber-50 dark:bg-amber-500/15", text: "text-amber-600 dark:text-amber-400", border: "border-amber-200 dark:border-amber-500/30" }
  }
  if (percentageUsed >= 40) {
    return { bg: "bg-blue-50 dark:bg-blue-500/15", text: "text-blue-600 dark:text-blue-400", border: "border-blue-200 dark:border-blue-500/30" }
  }
  return { bg: "bg-emerald-50 dark:bg-emerald-500/15", text: "text-emerald-600 dark:text-emerald-400", border: "border-emerald-200 dark:border-emerald-500/30" }
}
...
export default function RequiringActionPage() {
  ...
  return (
    <div className="w-full">
      ...
      {/* Error */}
      {error && !isLoading && (
        <div className="flex items-center gap-3 py-4 px-4 bg-red-50 dark:bg-red-500/15 rounded-xl border border-red-100 dark:border-red-500/30">
      ...
      {!isLoading && !error && items.length === 0 && (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          <div className="w-10 h-10 rounded-full bg-emerald-50 dark:bg-emerald-500/15 flex items-center justify-center mb-2">
            <CheckCircle2 className="w-5 h-5 text-emerald-500" />
          </div>
          <p className="text-sm font-semibold text-[#0C1D38] dark:text-white">No properties require action</p>
            ...
            {displayItems.map(({ property, system }, index) => {
              ...
              return (
                <div
                  ...
                  } hover:bg-orange-50/60 dark:hover:bg-white/5 transition-colors`}
                >
                  ...
                  <div className="min-w-0">
                    <h3 className="text-sm font-bold text-[#0C1D38] dark:text-white truncate" style={{ fontFamily: "Manrope, sans-serif" }}>
                      {property.address || property.property_name}
                    </h3>
                    <p className="text-xs text-gray-400 dark:text-gray-400 mt-0.5 truncate" style={{ fontFamily: "Manrope, sans-serif" }}>
                      {property.location}
                    </p>
                  </div>
                  <div className="w-[120px] flex items-center justify-center">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-[#0C1D38] dark:text-white truncate" style={{ fontFamily: "Manrope, sans-serif" }}>
                      {system.action_label}
                    </p>
                    <p className="text-xs text-gray-400 dark:text-gray-400 mt-0.5 truncate" style={{ fontFamily: "Manrope, sans-serif" }}>
                      {system.timeframe}
                  ...
                  <button
                    ...
                    className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-200 dark:hover:bg-white/10 text-gray-400 dark:text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors flex-shrink-0 disabled:opacity-50"
                    title="Dismiss"
                  >
                    {isDismissing ? (
