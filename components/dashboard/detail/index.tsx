...
import { processAPI, MessageResponse, documentAPI, DocumentResponse } from "@/lib/api"
...
export function PropertyDetail({ property }: PropertyDetailProps) {
    ...
    useEffect(() => {
        const fetchDocuments = async () => {
            ...
            try {
                const fetchedDocs = await documentAPI.getPropertyDocuments(property.id)
                // Map backend documents to display format
                const mappedDocs: DocumentItem[] = fetchedDocs.map((doc: DocumentResponse) => ({
                    id: doc.doc_id,
                    name: doc.doc_type === 'home_inspection' ? 'Home Inspection' : 
                          doc.doc_type === 'four_point' ? '4 Point Evaluation' : 
                          doc.doc_type === 'wind_mitigation' ? 'Wind Mitigation' : 
                          doc.doc_type,
                    progress: 100,
                    timestamp: "Uploaded",
                    icon: "check",
    ...
    if (!property) {
        return (
            <Card className="overflow-hidden py-0 shadow-none h-full flex items-center justify-center min-h-[200px]">
    ...
    return (
        <div className="flex flex-col bg-white rounded-tl-[32px] lg:rounded-none h-full relative overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                <div className="relative w-full h-[220px] md:h-[280px] bg-gradient-to-br from-sky-100 via-blue-50 to-emerald-50 overflow-hidden rounded-t-[32px] lg:rounded-none">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/40 to-transparent p-6">
                        <p className="text-xs text-white/90 font-medium drop-shadow-sm">{property.name}</p>
                    </div>
                </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-sm font-medium text-[#0C1D38]">
                                        {isLoadingDocuments ? "..." : `${documents.length} Submitted`}
                                    </span>
                                </div>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-sm font-medium text-[#0C1D38]">
                                        {property.status === "Completed" ? 100 : property.status === "In Progress" ? 50 : 0}% Complete
                                    </span>
                                </div>
                                <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                                    <div
                                        className="h-full bg-blue-500 rounded-full transition-all duration-300"
                                        style={{ width: `${property.status === "Completed" ? 100 : property.status === "In Progress" ? 50 : 0}%` }}
                                    />
                                </div>
                            </div>
                        </div>
